...
\headerbox{Materials \& Methods}{name=method,column=0,below=introduction,bottomaligned=conclusion}{ % This block's bottom aligns with the bottom of the conclusion block
  ...
  To deal with the imbalance, we applied \emph{SMOTE with Tomek Links} which applies the over-sampling method SMOTE combined with the under-sampling method Tomek links. The combination of both should re-balance the dataset while avoiding the creation of noisy samples from the interpolation of outliers and inliers.\\
...
\end{itemize}
  ...
  We focused on achieving the best \emph{F1-score} while keeping the \emph{recall} high.
